...
export default function CalendarPage() {
  ...
  const eventStyleGetter = (event: CustomEvent) => {
    ...
    if (event.type === 'reminder') {
      backgroundColor = 'rgba(217, 119, 6, 0.15)'; // amber-600/15
      textColor = '#fbbf24'; // amber-400
      border = '1px solid rgba(217, 119, 6, 0.3)';
    } else if (event.type === 'budget_due') {
      backgroundColor = 'rgba(5, 150, 105, 0.15)'; // emerald-600/15
  ...
  return (
    <div className="space-y-6 min-h-[80vh] flex flex-col">
      ...
      <div className="flex flex-wrap items-center gap-4 p-4 bg-slate-900/30 border border-slate-800/80 rounded-2xl backdrop-blur-sm text-xs font-semibold">
        <span className="text-slate-400">สัญลักษณ์กิจกรรม:</span>
        <div className="flex items-center gap-1.5 px-3 py-1 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400">
          <span className="w-2 h-2 rounded-full bg-amber-400 animate-ping" />
          <span>🔔 วันแจ้งเตือนความจำ (Reminder Dates)</span>
        </div>
        <div className="flex items-center gap-1.5 px-3 py-1 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400">
      ...
      <div className="flex-1 min-h-[550px] relative">
        {isLoading ? (
          ...
          <div className="h-full bg-slate-950/20 rounded-2xl">
            <BigCalendar
              localizer={localizer}
              events={events}
              startAccessor="start"
      ...
      {selectedEvent && (
        <div className="fixed inset-0 z-50 flex items-center justify-end">
          ...
          {/* Drawer Container */}
          <div className="relative w-full max-w-md h-full bg-slate-900 border-l border-slate-800 shadow-2xl p-6 flex flex-col justify-between z-10 animate-slide-in">
            <div>
              {/* Close and Title */}
              <div className="flex items-center justify-between pb-4 border-b border-slate-800/80 mb-6">
                <h2 className="text-lg font-bold bg-gradient-to-r from-violet-400 to-indigo-200 bg-clip-text text-transparent">
              ...
              <div className="mb-6">
                {selectedEvent.type === 'reminder' ? (
                  <div className="flex items-center gap-2 p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400">
                  ...
                  <div className="flex items-center gap-2 p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400">
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <p className="text-base font-bold text-slate-100">{selectedEvent.item.title}</p>
                </div>
                ...
                {selectedEvent.item.description && (
                  <div>
                    ...
                    <p className="text-xs text-slate-350 leading-relaxed bg-slate-950/40 p-3 rounded-xl border border-slate-800/40">
                      {selectedEvent.item.description}
                    </p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <span className={`inline-block px-2.5 py-0.5 rounded-full text-[10px] font-extrabold tracking-wide uppercase ${
                      selectedEvent.item.status === 'Pending' ? 'text-amber-400 bg-amber-500/10' :
                      selectedEvent.item.status === 'Purchasing' ? 'text-violet-400 bg-violet-500/10' :
            ...
            <div className="pt-4 border-t border-slate-800/80">
              <button
                onClick={() => setSelectedEvent(null)}
                className="w-full py-2.5 rounded-xl font-bold bg-slate-800 hover:bg-slate-750 text-slate-200 text-xs transition-all duration-200 cursor-pointer"
              >
